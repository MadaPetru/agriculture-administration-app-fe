...
export class TableOperationsHistoryProvider {
  ...
  public static getAttributeBasedOnColumn(column: string, element: TableOperationHistory): any {
    ...
    if (column === 'Estimated harvest') return element.estimatedHarvest + ' ' + element.estimatedHarvestMeasureType;
    ...
    if (column === 'Type of plant') return element.typeOfPlant;
    if (column === 'Estimated revenue') return element.estimatedRevenue + ' ' + element.estimatedRevenueCurrencyType;
  }
}
